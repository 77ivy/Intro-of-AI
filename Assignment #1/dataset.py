...
def load_data_small():
    ...
    train_list=[]
    for pgm_file in glob.glob("data/data_small/train/face/*.pgm"):
        img=Image.open(pgm_file)
        img_array=np.array(img.getdata())
        img_array.resize((19,19))
        img_tuple=img_array,1
        train_list.append(img_tuple)
    for pgm_file in glob.glob("data/data_small/train/non-face/*.pgm"):
        img=Image.open(pgm_file)
        img_array=np.array(img.getdata())
        img_array.resize((19,19))
        img_tuple=img_array,0
        train_list.append(img_tuple)

    #load test file in test_list
    test_list=[]
    for pgm_file in glob.glob("data/data_small/test/face/*.pgm"):
        img=Image.open(pgm_file)
        img_array=np.array(img.getdata())
        img_array.resize((19,19))
        img_tuple=img_array,1
        test_list.append(img_tuple)
    for pgm_file in glob.glob("data/data_small/test/non-face/*.pgm"):
        img=Image.open(pgm_file)
        img_array=np.array(img.getdata())
        img_array.resize((19,19))
        img_tuple=img_array,0
...
def load_data_FDDB(data_idx="01"):
    ...
    while line_idx < len(line_list):
        ...
        for i in range(num_faces):
            ...
            left,right = face_box_list[i]
            w = right[0] - left[0]
            h = right[1] - left[1]
            # 下
            if(left[1] + h <= img_gray.shape[0] and right[1] + h <= img_gray.shape[0]):
                img_crop = img_gray[left[1]+h:right[1]+h, left[0]:right[0]].copy() #y,x
            elif left[1] - h >= img_gray.shape[0] and right[1] + h <= img_gray.shape[0]:
                img_crop = img_gray[left[1]-h:right[1]-h, left[0]:right[0]].copy() #y,x

...
class Adaboost:
    ...
    def selectBest(self, featureVals, iis, labels, features, weights):
        ...
        for idx, feature in enumerate(featureVals):
            ...
            w_pos, w_neg = 0, 0
            best_error, bf, bth, bp = float('inf'), 0, 0, 0
            for w, f, label in applied_feature:
                error = min(w_neg +all_w_pos- w_pos, w_pos +all_w_neg- w_neg)
            ...
            clf = WeakClassifier(bf, bth, bp)
            classifiers.append(clf)

        bestClf, bestError = 0, float('inf')
        for clf in classifiers:
            error = 0
            for data, w, las in zip(iis, weights, labels):
                correctness = abs(clf.classify(data) - las)
                error += w * correctness 
            error = error / 200        
            if error < bestError:
